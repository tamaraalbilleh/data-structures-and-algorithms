...
const screenForNames = (arr) => {
  // Solution code here...
  let finalArray =[];
  let regex1 = /^Mr.\s[a-zA-Z]/g;
  let regex2 = /^Mrs.\s[a-zA-Z]/g;
  let regex3 = /^Ms.\s[a-zA-Z]/g;
  let regex4 = /^Dr.\s[a-zA-Z]/g;
  arr.map (item=>{
    if (regex1.test(item) || regex2.test(item) || regex3.test(item) || regex4.test(item) ){
...
------------------------------------------------------------------------------------------------ */
const helpCheck = function (c1,c2,c3){
  if (c1 === '' && c2 === '' && c3 === ''){
    return false ;
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i=0;i<board.length;i++){
    if(helpCheck(board[i][0] ,board[i][1],board[i][2])){
      console.log ('hi');
      return true;
    }else if (helpCheck(board[0][i] ,board[1][i],board[2][i])){
      return true;
    }
  }
  if (helpCheck(board[0][0] ,board[1][1],board[2][2])){
    return true;
  }else if (helpCheck(board[0][2] ,board[1][1],board[2][0])){
    return true;
  }
